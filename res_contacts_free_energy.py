...
def native_contacts_total(residues1, residues2, cutoff2, contacts, box):
...
total_ref_sel = loos.selectAtoms(reference,total_contact_string)
total_ref_sel_residues= total_ref_sel.splitByResidue()
total_natives = find_native_contacts(total_ref_sel_residues, ref_sel2_residues,cutoff)
#print natives
...
total_sel = loos.selectAtoms(model,total_contact_string)
total_sel_residues = total_sel.splitByResidue()

"####WESTPA specific information #####"
root_dir=system + "/"+"traj_segs"
westh5 = system + "/" + "west.h5"
iter = []
outputh5file = system + "/"+ str(sys.argv[7])
iter_sorted = []
...
for i in iter_sorted:
        ...
        t = int(i)
        seg = []
        data = numpy.empty([len(os.listdir(iter_dir)),11])
        pcoord = numpy.empty([len(os.listdir(iter_dir)),11])
        ...
        for c in seg_dir:
            ...
            for frame in traj:
                    ...
                    pcoord[l][k] =  native_contacts_total(total_sel_residues,sel2_residues,cutoff2,total_natives,box)
                    k=k+1
        
        name_ds='iter'+'_'+'00'+ i
#        weights = 'iterations'+'/'+name_ds+'/'+'weight'
#        wt_data = west[weights]
#        wt = numpy.array(wt_data)
...
#       print final.shape
        ...
        s.attrs['n_iter']= t
        s.create_dataset("res_cont",shape=(len(os.listdir(iter_dir)),11,2),data=final)
f.close()
